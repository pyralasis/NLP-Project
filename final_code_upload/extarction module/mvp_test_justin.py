...
from visualize_dataset import plot_class_distribution, plot_combined_class_distribution, plot_aggregated_class_distribution
...
def main():
    ...
    if(False):
        ...
        plot_class_distribution(processed_dataset, id2label, "training & testing")
        plot_combined_class_distribution(processed_dataset, id2label, "combined opinions")
        plot_aggregated_class_distribution(processed_dataset, id2label, "combined B- & I-")
    ...
    final_predictions = trainer.predict(evaluation_dataset)
    final_labels = final_predictions.label_ids.flatten()
    final_preds = final_predictions.predictions.argmax(-1).flatten()

   
       # Filter out invalid labels like -100
    unique_labels = sorted(label for label in set(final_labels) | set(final_preds) if label != -100)

    # Map unique labels to their display names (e.g., from id2label)
    display_labels = [id2label[label] for label in unique_labels if label in id2label]

    # Compute confusion matrix
    cm = confusion_matrix(final_labels, final_preds, labels=unique_labels)

    # Normalize confusion matrix (handle zero rows)
    ...
    plt.title("Normalized Confusion Matrix - Final Evaluation")
    plt.show()

    # Final evaluation metrics
    final_eval_metrics = {
        "accuracy": accuracy_score(final_labels, final_preds),
        "precision_recall_fscore_support": precision_recall_fscore_support(final_labels, final_preds, average="weighted"),
        "confusion_matrix": cm.tolist(),
        "classification_report": classification_report(
            final_labels, 
            final_preds, 
            labels=unique_labels,  # Use unique_labels to match the confusion matrix
            target_names=display_labels,  # Use display_labels for the report
            zero_division=0, 
            output_dict=True
        )
    }
    ...
    print(json.dumps(final_eval_metrics, indent=4))
    ...
    if save_results:
        ...
        with open(os.path.join(MODEL_DIR, "final_eval_metrics.json"), "w") as f:
